...
// announce_templates のテスト: フォールバック連鎖（remote→永続キャッシュ→builtin）・TTL・
// 不正エントリのスキップ・templateId 解決・PUT 検証・orchestrator の実行時解決。
// PUT は favorites_test と同様 ResponseRecorder でハンドラ直叩き（requireAuth を経由しない）。
...
// newTplServer は dataDir=tmp の driver 未起動 Server を返し、テンプレ取得元を remote へ差し替える。
// remote=nil は確実に失敗する取得元（クローズ済みサーバー）＝オフライン相当。
...
func TestAnnounceTemplates_RemoteSuccessAndTTL(t *testing.T) {
	calls := 0
	s, dir := newTplServer(t, func(w http.ResponseWriter, _ *http.Request) {
		calls++
		_, _ = w.Write([]byte(tplJSON))
	})
	list, source := s.announceTemplates(context.Background())
	...
	if _, _ = s.announceTemplates(context.Background()); calls != 1 {
		t.Fatalf("TTL 内で再取得された: calls=%d", calls)
	}
}

func TestAnnounceTemplates_FallbackToPersisted(t *testing.T) {
	s, dir := newTplServer(t, nil) // 取得は必ず失敗
	writeJSONFile(filepath.Join(dir, "announce-templates.json"), announceTemplateList{
		Version:   1,
		Templates: []announceTemplate{{ID: "saved", URL: "resrec:///saved", Tag: "T"}},
	})
	list, source := s.announceTemplates(context.Background())
...
func TestAnnounceTemplates_FallbackToBuiltin(t *testing.T) {
	...
	if _, ok := s.lookupAnnounceTemplate(context.Background(), id); !ok {
		t.Fatalf("既定 templateId %q がビルトインに無い（同期切れ）", id)
	}
}

func TestAnnounceTemplates_InvalidEntriesSkipped(t *testing.T) {
...
func TestAnnounceTemplates_AllInvalidFallsBack(t *testing.T) {
	s, _ := newTplServer(t, func(w http.ResponseWriter, _ *http.Request) {
		_, _ = w.Write([]byte(`{"version":1,"templates":[{"id":"","url":"","tag":""}]}`))
	})
	if _, source := s.announceTemplates(context.Background()); source != "builtin" {
