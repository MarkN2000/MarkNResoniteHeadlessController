...
1. **ワールドごとのポートを固定する** — Web UI の「コンフィグ」タブでワールドの上級設定を開き、`LNL 固定ポート`、`QUIC 固定ポート`、`TCP 固定ポート` のうち必要なものを設定して保存します。空欄のプロトコルはランダムポートになります。QUIC は複数ワールドで同じポートを共有できないため、起動対象の各ワールドに固有の番号を指定してください。
2. **LAN 外で QUIC を使う場合はパブリック IP を設定する** — 「設定」タブの「QUIC 設定」にサーバーの実際の IPv4 または IPv6 を入力します。これは Resonite Headless の次回起動から反映されます。LAN 内だけの QUIC 接続では不要です。
