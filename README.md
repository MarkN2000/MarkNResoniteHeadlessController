...
#### ⚠️ 残りタスク（優先度低）
1. **ユーザー0監視の定期チェック統合** - WebSocketまたはポーリング実装が必要
2. **予定再起動のカード機能** - UI/UX向上（後回し可）
3. **チャットメッセージの個別セッション対応** - 現在は全体ブロードキャスト
4. **アイテムスポーンの詳細設定** - アイテムURLの管理機能
