...
**4.（任意）セッションへの直接参加を速くする — プロトコル別ポートの設定**
...
1. **ワールドごとのポートを固定する** — Web UI の「コンフィグ」タブでワールドの上級設定を開き、`LNL 固定ポート`、`QUIC 固定ポート`、`TCP 固定ポート` のうち必要なものを設定して保存します。空欄のプロトコルはランダムポートになります。LNL と QUIC はどちらも UDP のため、起動対象の全ワールドを通して異なる番号を指定してください。TCP は別トランスポートなので同じ番号を使用できます。
2. **LAN 外で QUIC を使う場合はパブリック IPv4 を設定する** — 「設定」タブの「QUIC 設定」にサーバーの実際の IPv4 アドレスを入力します。Resonite 2026.8.7.887 では IPv6 を指定するとワールド起動に失敗するため使用できません。これは Resonite Headless の次回起動から反映されます。LAN 内だけの QUIC 接続では不要です。
3. **クラウド側を開放する** — Oracle Cloud コンソール → 対象 VCN の「セキュリティリスト」（または NSG）→ イングレス規則を追加します。LNL と QUIC は `UDP`、TCP は `TCP` で、設定した宛先ポート（例: `32100`）を開放します。
4. **VM 内（Ubuntu）を開放する** — Oracle の Ubuntu は raw iptables が有効です（ufw は既定で無効）。次は UDP `32100` の例です。
...
- **Web UI から** — 新しいバージョンがあると、画面右上の ⋮ に赤丸が付きます。⋮ →「更新を確認」→「アップデート」で、ダウンロード（進捗バー表示）・検証・差し替えまで自動で行われ、**MRHC を再起動した時から** 新バージョンになります（差し替え自体は稼働中のワールドに影響しません）。続けて「今すぐ再起動する」を押すとワールドを順に停止してから MRHC が新バージョンで自動的に起動し直し、画面も自動で再接続します（停止に時間がかかると数分かかることがあります）。
...
- **ヘッドレスのログを見たい** — Web UI の「ログ」タブで、Resonite ヘッドレスのログファイル（`<インストール先>/Headless/Logs`）を選んで表示・コピー・ダウンロードできます（読み取り専用・停止中でも閲覧可能）。大きいログは末尾のみ表示・コピーされますが、ダウンロードでは元ファイルの全文を保存します。稼働中の現行ログは、OS によっては読み取れないことがあります。
...
**起動・停止・状態監視**
- ヘッドレス（Resonite プロセス）の起動／通常停止（告知して締め切り、全員退出で即・最長1分で停止）／強制停止／再起動
- 稼働状況のライブ表示：参加者（在席／離席）、ワールド情報、稼働時間、セッション URL
- Resonite 出力のライブログ（SSE）と、ログファイルの閲覧・コピー・ダウンロード
...
- 公式スキーマ準拠でワールド単位に詳細設定（アクセスレベル・タグ・AFK キック・自動保存・自動復帰・ロール・自動招待・プロトコル別の `forcePorts` など）
