...
2. **LAN 外で QUIC を使う場合はパブリック IP を設定する** — 「設定」タブの「QUIC 設定」にサーバーの実際の IPv4 または IPv6 を入力します。IPv6 の角括弧は MRHC が保存時に自動付与します。これは Resonite Headless の次回起動から反映されます。LAN 内だけの QUIC 接続では不要です。
