...
- 停止中: セッション/フレンド/コマンド/新規セッションは「起動してください」表示。コンフィグ/スケジュール/設定/**ログ**はファイル（ディスク上のログ）を読むので使える
...
- **ワールド検索枠（右・2026-06-04 実装）**: キーワード入力（Enter可）→ `GET /api/v1/resonite/worlds?q=`（`internal/resonite.SearchWorlds`＝go.resonite.com プロキシ）→ 2カラムグリッド（サムネ `Image`／空はプレースホルダ＋名前＋所有者ID）。各カードに **[起動]**＝`useConfirm`+`ConfirmModal`→`startWorldURL(resoniteUrl)`→`onStarted`（左 StartPanel と同形）。状態は **loading / 結果 / 該当なし の3つ**（独立エラー状態なし＝`getData` の null→[] で失敗を「該当なし」に吸収・フレンド検索と同流儀）。`reqId` で連打/順序逆転ガード。
  ...
  - **お気に入り（2026-06-04 実装）**: 検索結果/お気に入りカードに ★/☆ トグル（`ActionIcon`・無音・確認なし＝非破壊可逆）、検索ボタン隣の ★ で「お気に入り表示↔検索結果」を切替。サーバー保存 `favorites.json`（dataDir直下・`favMu` 直列化・0600・`runtime-state.json` と共通の汎用ヘルパ `readJSONFile`/`writeJSONFile`＝`jsonstate.go`）。同一キー=`recordId`・冪等・上限100。`GET/POST/DELETE /api/v1/favorites`（add/remove は更新後一覧を返す）。型は `WorldResult` 再利用。テスト=`favorites_test.go`。
...
- **プロトコル別固定ポート（2026-08-08、実機確認で補訂）**: 現行形式は `forcePorts` に一本化し、ワールド上級設定へ `lnl`・`quic`・`tcp` の3行を常時表示する。各値は整数 `1..65535`、空欄はそのプロトコルを辞書から削除する。UI操作で3プロトコルとも空になり、未知プロトコルも無ければ `forcePorts` 自体を削除する。プロトコルが辞書に無い場合は Resonite が設定範囲内のランダムポートを使う。QUIC 等の有効/無効スイッチは追加しない。辞書内の未知プロトコルは編集時も温存する。LNL と QUIC はともに UDP リスナーを使うため、起動対象（`isEnabled != false`）の全ワールドを通して `forcePorts.lnl` と `forcePorts.quic` の固定ポートを重複させない。同一ワールドの LNL/QUIC 間、別ワールド間、プロトコルをまたぐ重複のいずれも保存を抑止する。旧版や手編集で重複した保存済みコンフィグは、ヘッドレス起動前に競合エラーとして拒否し、元ファイルは変更しない。TCP は別トランスポートなので LNL/QUIC と同じ番号を許容する。
...
- **`config.WaitControl` を使うのは予定再起動のみ**。**手動「通常再起動」と通常停止（R7）は固定の即応**＝`quietWaitMin=0`/`announceWaitMin=1`（即告知＋最長1分・`orchestrator.immediateManualWait`）で上書きする。意図: 管理者がボタンを押したときは「今すぐ寄り」で締め切りたいため、長い予告（既定58+2分）は予定再起動だけに適用する。0人なら即／全員退出で即（在席監視は `waitInterval` 間隔ポーリング）。
...
### 3.17 キャッシュ管理（設定タブ・リリース後追加）の確定仕様

設定タブに「キャッシュ管理」セクションを追加。対象は**既定キャッシュ `{dataDir}/headless-cache` のみ**（`hlconfig.DefaultFolders` と同一導出＝コンフィグに焼き込む値と一致。独自 `cacheFolder` を設定した場合は対象外＝既定運用前提・UI にパスを明示して透明化）。

- **停止時の自動「古いファイル削除」**（オプトイン）: トグル ON のとき、ヘッドレス**停止時**に最終更新(mtime)が `maxAgeDays`（既定30・最小1）より古いキャッシュファイルを自動削除する。設定は `config.CacheCleanup{enabled,maxAgeDays}`（追加フィールド＝**マイグレ不要**・未設定=OFF/30日＝`CacheCleanupOrDefault`）。
  - **判定基準は mtime（最終更新）**。atime（最終アクセス＝使用）は Windows で既定無効化されがち／Linux relatime で近似と**クロスプラットフォームで信頼できない**ため不採用。キャッシュは write-once 運用が多く mtime≒キャッシュ日時だが、消えても必要なら次回再取得されるため妥当（2026-06-08 実機の単一キャッシュファイルで created=modified=accessed 一致を観測＝読み取りで touch しない傍証）。
  - **トリガー=停止時**（起動時でなく）: 停止は `driver.waitExit` の1地点に集約され、プロセス reap 済み＝キャッシュ確実に未使用で安全。起動経路は4箇所（手動/ランタイムガード/orchestrator/crash）に散る。Driver に **`SetOnStopped` コールバック**を追加し、`waitExit` の **通常停止(`wasStopping`)・クラッシュ(`default`)** で発火（**重複インスタンス失敗時は呼ばない**＝孤児が使用中の恐れ）。クラッシュ自動復帰(`onExit`)の**前**に同期実行＝復帰起動と削除の競合回避。結果は sys ログへ i18n 表示（削除>0 のときのみ）。
- **サイズ表示**: パスは即表示（`/headless-config-defaults` の cacheFolder＝走査不要）。合計サイズは**走査するため「サイズを計算」ボタン押下時のみ** `GET /cache/info` を呼ぶ（巨大キャッシュでも他操作を止めない）。
- **手動全削除**: 「全キャッシュ削除」ボタン（赤・確認ダイアログ）。**ヘッドレス停止中のみ**（`State!=stopped` は 409）。中身を空にし（フォルダ自体は残す＝次回起動の EnsureFolders で再利用）。
- **並行性**: 自動削除と手動全削除は `cacheMu` で直列化（停止直後の自動削除と手動削除の同時実行を防ぐ）。削除コア（`evictOlderThan`/`emptyDir`）は純関数・os 標準のみ＝Win/Linux 共通・テスト容易。
- **API**: `GET/PUT /api/v1/cache/config`（トグル/日数）・`GET /api/v1/cache/info`（path/sizeBytes/exists）・`POST /api/v1/cache/clear`（停止中のみ）。

### 3.18 ログ閲覧タブ（リリース後追加）の確定仕様

Resonite ヘッドレスのログファイルを Web UI で閲覧する**読み取り専用**タブ（編集なし）。診断用途のため**稼働中/停止中どちらでも使える**（`availableWhenStopped: true`・タブ順はコマンドの直後）。
