...
## 追加計画: ワールド検索して起動（go.resonite.com スクレイピング）

目的: ユーザーがキーワードで公開ワールドを検索し、カード一覧から選択して「起動」できるようにする。

背景/制約:
- CORS回避のため検索はバックエンドで実行（スクレイピング）。
- 公式のHTML構造変更に弱いため、例外処理とフォールバックを用意。短期TTLキャッシュを推奨。

機能要件:
- キーワード入力 → 検索 → 画像+タイトルのカードで一覧表示。
- カードクリックで選択（選択スタイル）。
- 「このワールドを起動」ボタンで `startworldurl resonite:///world/R-xxxx` を実行。
- 画像なし時はプレースホルダ表示。

API仕様（バックエンド）:
- `GET /api/world-search?term=<keyword>`
  - 取得先: `https://go.resonite.com/world?term=<keyword>`
  - 解析対象: `ol.listing > li > a.listing-item`
  - 抽出: name, imageUrl, recordId, resoniteUrl
  - キャッシュ: termキーで短期TTL（例: 60秒）。

フロント仕様（Svelte、newWorld タブ右カラム）:
- ダッシュボード準拠のUI（`panel-heading`, `card`, `status-form`, `field-row`）。
- 入力+検索ボタン、ローディング/エラー表示。
- 結果はカードグリッド、選択可能。選択中は強調枠表示。
- 「このワールドを起動」ボタン（選択時のみ活性）。

非機能要件:
- 入力サニタイズ（長さ/空白）。
- タイムアウト/リトライ、失敗時のユーザーフィードバック。
- ログ最小限（PIIを含めない）。

実装手順（高レベル）:
1) バックエンドに `GET /api/world-search` を追加（cheerioでHTML解析、短期キャッシュ）。
2) 共有クライアントにAPI関数追加（`getWorldSearch(term)`）。
3) フロントに状態/アクション/カードUI追加、起動ボタンは `postCommand('startworldurl <url>')`。
4) ローディング・エラー・空結果ハンドリング、画像フェール時のプレースホルダ。
5) 簡易E2E確認（代表キーワード、選択→起動の一連）。